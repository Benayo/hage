...
const SubscriptionCard = ({ plans }) => (
    ...
    {plans.map((plan, index) => (
      <div
        key={index}
        className={`p-8 rounded-xl flex flex-col h-full ${plan.bgColor}`}
      >
        {/* Header */}
        ...
        <div className="border-t-[0.0625rem] border-secondary-200 py-8 flex-grow">
          ...
          {plan.features.map((feature, i) => (
            <div
              key={i}
              className={`flex items-center my-4 ${plan.textColor}`}
            >
        ...
        <div className="mt-6 md:mt-auto">
          <button
            className={`py-3 w-full text-white-100 rounded-lg text-center ${plan.buttonBg}`}
          >
            Contact Us
          </button>
        </div>
